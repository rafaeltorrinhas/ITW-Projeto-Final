    ...
    self.activate = function () {
      ...
      ajaxHelper(composedUri, 'GET').done(function (data) {
          ...
          if (JSON.parse(window.localStorage.getItem('favStates0')) == null) {
              ...
              console.log('checking which States were favourited')
              var statesList = [];
              var favStatesList = JSON.parse(window.localStorage.getItem('favStates0'));
              var a = favStatesList.length;
              console.log(a,favStatesList)
              for (var i = 0; i < a; i++) {
                  console.log(favStatesList[i])
                  statesList.push(favStatesList[i])
              }
              self.records(statesList)
          }
          hideLoading();
          a = JSON.parse(window.localStorage.getItem('favStates0'));
