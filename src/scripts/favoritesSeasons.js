...
var vm = function () {
    ...
    self.favoriteSeason = function (id) {
        ...
        if (JSON.parse(window.localStorage.getItem('favSeasons0')) == null) {
            console.log('no favSeasons in local storage, lets create it');
            window.localStorage.setItem('favSeasons0', '[]');
            var a = JSON.parse(window.localStorage.getItem('favSeasons0'));
            for(var i=0;i<self.records().length;i++){
                if(self.records()[i].Id == id){
                b = a.concat(self.records()[i]);
            }}
            window.localStorage.setItem('favSeasons0', JSON.stringify(b));
        } else {
            var c = JSON.parse(window.localStorage.getItem('favSeasons0'))
            for (var i = 0; i < c.length; i++) {
                if (id == c[i].Id) {
                    c.splice(i, 1); // remove the item at index i
                    window.localStorage.setItem('favSeasons0', JSON.stringify(c)); // update the local storage
                    console.log('Season unfavourited')
            ...
            window.localStorage.setItem('favSeasons0', JSON.stringify(b));
            console.log('Season not favourited, added to favourites')
        }
        console.log(JSON.parse(window.localStorage.getItem('favSeasons0')))
    }
    ...
    self.search = function () {
        ...
        console.log('CALL: searchSeasons...');
